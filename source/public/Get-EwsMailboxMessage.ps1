function Get-EwsMailboxMessage {
    ...
    # $folderToProcess = @()
    foreach ($f in $Folder) {
        if ($f.FolderClass -ne 'IPF.Note') {
            Write-Warning "The specified folder object [$($f.DisplayName)] if not a message folder (IPF.Note) type and will not processed."
            continue
        }
        ...
        do {
            if ($PSCmdlet.ParameterSetName -eq 'DateFilter') {
                $FindItemResults = $service.FindItems($f.Id, $SearchFilter, $ItemView)
            }
            else {
                $FindItemResults = $service.FindItems($f.Id, $ItemView)
            }

            if ($FindItemResults) {
                $FindItemResults | Add-Member -MemberType NoteProperty -Name Path -Value $f.Path
                $FindItemResults | Add-Member -MemberType NoteProperty -Name Folder -Value $f.DisplayName
                $FindItemResults | Add-Member -MemberType NoteProperty -Name MailboxAddress -Value $f.MailboxAddress
                $FindItemResults | Add-Member -MemberType NoteProperty -Name MailboxType -Value $f.MailboxType
    ...
    # $result | Add-Member -MemberType NoteProperty -Name Path -Value $f.Path
    # $result | Add-Member -MemberType NoteProperty -Name Folder -Value $f.DisplayName
    @($result)
}